...
- Deploy with this repository root; `vercel.json` uses `@vercel/python` build mapping and routes all requests to `app.py`.
...
## Troubleshooting: `Function Runtimes must have a valid version`

If Vercel shows this error, your `vercel.json` likely has an invalid `functions.*.runtime` value (for example `python3.12`).

Use this stable config instead:
...
## Step-by-step: get your Vercel deployment green

1. **Update `main` with the latest fixes** (important):
   ```bash
   git checkout main
   git pull origin main
   ```
2. **Verify dependency file does NOT contain `secure-filename`** and DOES include Werkzeug:
   ```bash
   ...
   - `python-dotenv`
   - `werkzeug`
3. **Verify app import uses Werkzeug**:
   ```bash
   rg "werkzeug.utils" app.py
   ```
4. **Commit and push if your local `main` changed**:
   ```bash
   git add requirements.txt app.py vercel.json README.md
   git commit -m "Fix Vercel deploy: use Werkzeug and supported Python builder"
   git push origin main
   ```
5. **In Vercel → Deployments**, redeploy the latest `main` commit.
6. **In Vercel → Settings → Environment Variables**, confirm:
   - `SENDER_EMAIL`
   - `SENDER_PASSWORD`
   - `FLASK_SECRET_KEY`
7. **Health check**:
